...
double compute_StanleySteering(int idx) { // <- 여기서 dynamic ld에 의한 idx를 넣는게 의도였음,,
  int N = static_cast<int>(g_path_xy.size());
  if (N < 2 || idx < 0 || idx >= N) {
    return 0.0;
  }
  ...
  double delta = psi_err + std::atan2(g_k_stanley * e_y, v); // 횡오차, 헤딩오차 --> 하나,,?
  return delta;
}
...
int main(int argc, char **argv) {
  ...
  nh.param<double>("Kp_lat",        g_Kp_lat,      0.2);
  nh.param<double>("Ki_lat",        g_Ki_lat,      0.0);
  nh.param<double>("Kd_lat",        g_Kd_lat,      0.02);
  nh.param<double>("steer_alpha",   g_alpha_steer, 0.4);
