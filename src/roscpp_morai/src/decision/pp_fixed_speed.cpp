...
public:
  PPFixedSpeedNode()
  : pnh_("~"), lc_(GeographicLib::Geocentric::WGS84()) {

    // ---- 파라미터 ----
    pnh_.param<std::string>("ref_file",   ref_file_,   std::string("/root/ws/src/roscpp_morai/map/ref.txt"));
    pnh_.param<std::string>("path_file",  path_file_,  std::string("/root/ws/src/roscpp_morai/map/Path.txt"));
    pnh_.param<double>("wheelbase",       wheelbase_L, 3.0);       // [m]
    pnh_.param<double>("lookahead",       lfd_,        4.5);       // [m]
    pnh_.param<double>("target_kmh",      target_vel_kmh, 15.0);   // [km/h]

    pnh_.param<double>("avoid_kmh",       avoid_kmh_,  8.0);
    pnh_.param<std::string>("flag_topic", flag_topic_, std::string("/avoid_flag"));
    ...
    flag_sub_ = nh_.subscribe<std_msgs::UInt8>(flag_topic_, 10, &PPFixedSpeedNode::flagCB, this);
    ...
    while (ros::ok()) {
      ...
      if (mode_ == Mode::AVOID_LEFT || mode_ == Mode::AVOID_RIGHT) {
        ...
        ROS_INFO_THROTTLE(0.5,
          ...
          enu_x_, enu_y_, yaw_*180.0/M_PI, cmd_.steering, cmd_.velocity, nearest_idx_tmp);
        rate.sleep();
        continue;
      }
      ...
      {
        const double cosv = std::cos(yaw_), sinv = std::sin(yaw_);
        ...
        if (!found) {
          ...
          lx =  cosv*dx + sinv*dy;  ly = -sinv*dx + cosv*dy;
          found = (lx > 0.0);
        }
      }
      ...
      cmd_.velocity = target_vel_kmh; // [km/h] (원본 유지)
      cmd_.accel = 0.0; cmd_.brake = 0.0;
...
private:
  ...
  void egoCB(const morai_msgs::EgoVehicleStatus &msg) {
    ego_speed_ms_ = msg.velocity.x; // 로그용
  }

  void flagCB(const std_msgs::UInt8::ConstPtr& m) {
    const uint8_t f = m->data;
    switch (f) {
      case 0: mode_ = Mode::TRACK_WAYPOINT;                 avoid_first_step_hard_right_ = false; break;
      case 1: mode_ = Mode::AVOID_LEFT;                     /* keep */ break;
      case 2: mode_ = Mode::AVOID_RIGHT;                    /* keep */ break;
      case 3: mode_ = Mode::AVOID_RIGHT; avoid_first_step_hard_right_ = true; break;
      default: break;
    }
  }
...
private:
  ...
  // pubs/subs
  ros::Subscriber gps_sub_, imu_sub_, ego_sub_;
  ...
  // state
  bool have_gps_{false}, have_yaw_{false};
  ...
  std::string ref_file_, path_file_, flag_topic_;

  // control params
  double wheelbase_L{3.0};
  double lfd_{4.5};
  double target_vel_kmh{15.0};
  double avoid_kmh_{8.0};

  // 회피 상태
  Mode mode_;
  bool avoid_first_step_hard_right_;
};
