...
// (추가) cav L-shape(OBB) 마커 퍼블리셔
Publisher pub_cav_lshape_boxes;

// (추가) cav L-shape 중심 PoseArray 퍼블리셔 (선택)
Publisher pub_cav_lshape_centers;

static inline geometry_msgs::Quaternion yawToQuat(double yaw_rad)
{
...
void callback(const sensor_msgs::PointCloud2ConstPtr& input_cloud)
{
    ...
    *roi_cloud += *x_outside;
    ...
    // const auto features = ExtractCluster2DFeatures(ransac_cloud, cluster_indices[i]);
    // ROS_INFO("[Convex] cluster=%d ... A_hull=%.3f best(score=%.3f, theta=%.1f deg ...)",
    //      i, features.hull_area, features.best_box.score, features.best_box.theta * 180.0 / M_PI);
    //      i, features.hull_area, features.best_box.score, features.best_box.theta * 180.0 / M_PI);
    ...
    // (추가) cav L-shape 중심들 PoseArray
    geometry_msgs::PoseArray lshape_centers_msg;
    lshape_centers_msg.header = input_cloud->header;
    if (lshape_centers_msg.header.frame_id.empty())
        lshape_centers_msg.header.frame_id = "lidar_link";

    // (추가) cav L-shape 박스(윤곽) MarkerArray
    visualization_msgs::MarkerArray lshape_boxes_msg;

    // L-shape fitter (delta_deg, d0)
    // - delta_deg는 내부적으로 후보 각도를 "hull edge"로 쓰기 때문에 크게 영향 없지만,
    //   d0는 closeness에서 바닥값 역할(너 코드의 기본값 0.15 유지)
    LShapeClosenessFitter lshape_fitter(5.0, 0.15);
    ...
    for (int i = 0; i < (int)cluster_indices.size(); ++i) {
        ...
        // -------------------------------------------------------
        // (추가) L-shape fitting (Convex Hull 기반) + RViz 박스
        // -------------------------------------------------------
        {
            std::vector<Point2D> cluster2d;
            cluster2d.reserve(cluster_indices[i].indices.size());

            for (int idx : cluster_indices[i].indices) {
                const auto& p = ransac_cloud->points[idx];
                cluster2d.push_back(Point2D{static_cast<double>(p.x), static_cast<double>(p.y)});
            }

            Rectangle rect;
            if (lshape_fitter.fit(cluster2d, rect)) {
                // 사각형 중심
                Eigen::Vector2d c2(0.0, 0.0);
                for (int k = 0; k < 4; ++k) c2 += rect.corners[k];
                c2 *= 0.25;

                const double zc = 0.5 * (d_i.min_pt.z() + d_i.max_pt.z());

                // 중심 pose (선택적으로 publish)
                geometry_msgs::Pose lpose;
                lpose.position.x = c2.x();
                lpose.position.y = c2.y();
                lpose.position.z = zc;
                lpose.orientation = yawToQuat(rect.heading);
                lshape_centers_msg.poses.push_back(lpose);

                // 박스 크기(회전박스): corner 거리로 계산
                const double w = (rect.corners[1] - rect.corners[0]).norm();
                const double l = (rect.corners[2] - rect.corners[1]).norm();
                const double h = std::max(0.05f, d_i.size.z());

                visualization_msgs::Marker obb;
                obb.header = centers_msg.header;
                obb.ns = "cav_lshape";
                obb.id = lshape_marker_id++;
                obb.type = visualization_msgs::Marker::CUBE;
                obb.action = visualization_msgs::Marker::ADD;
                obb.pose = lpose;
                obb.scale.x = std::max(0.05, w);
                obb.scale.y = std::max(0.05, l);
                obb.scale.z = h;
                obb.color.r = 1.0f;
                obb.color.g = 0.0f;
                obb.color.b = 0.0f;
                obb.color.a = 0.25f;
                obb.lifetime = ros::Duration(0.2);

                lshape_boxes_msg.markers.push_back(obb);

                ROS_INFO("[LShape] cluster=%d score=%.3f heading=%.1fdeg w=%.2f l=%.2f",
                         i, rect.score, rect.heading * 180.0 / M_PI, w, l);
            }
        }
    ...
    // (추가) cav L-shape centers/boxes publish
    pub_cav_lshape_centers.publish(lshape_centers_msg);
    pub_cav_lshape_boxes.publish(lshape_boxes_msg);
...
int main(int argc, char** argv)
{
    ...
    // (추가) cav L-shape 중심/박스
    pub_cav_lshape_centers = nh.advertise<geometry_msgs::PoseArray>("/cav_lshape_centers", 1);
    pub_cav_lshape_boxes   = nh.advertise<visualization_msgs::MarkerArray>("/cav_lshape_boxes", 1);
