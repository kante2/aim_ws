...
float inflation_m = 0.0f;      // AABB inflation (m)
                               ...
                               const std_msgs::Header &hdr)
{
  ...
  // 로컬 costmap: 프레임 원점(0,0)을 맵 중앙에 두기
  ...
  // 기본을 free로 깔고, 박스 영역만 cost를 올리는 방식
  cm.data.assign(w * h, free_cost);
}
...
// 지면 제거(간단 버전): z 임계치
static pcl::PointCloud<pcl::PointXYZI>::Ptr removeGroundByHeight(
  const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud)
{
  pcl::PointCloud<pcl::PointXYZI>::Ptr out(new pcl::PointCloud<pcl::PointXYZI>);
  out->reserve(cloud->size());

  const float ground_threshold = 0.05f; // 5cm 이상만 유지
  for (const auto &p : cloud->points) {
    if (p.z > ground_threshold) out->push_back(p);
  }
  out->width  = (uint32_t)out->size();
  out->height = 1;
...
void lidarCallback(const sensor_msgs::PointCloud2ConstPtr &in_msg)
{
  ...
  // 최종 출력 헤더는 "항상 costmap_frame"로 통일
  std_msgs::Header out_hdr;
  out_hdr.stamp    = in_msg->header.stamp;
  out_hdr.frame_id = costmap_frame;

  // 1) TF transform cloud -> costmap_frame
  sensor_msgs::PointCloud2 cloud_tf;

  if (use_tf) {
    const std::string &src_frame = in_msg->header.frame_id;

    if (src_frame != costmap_frame) {
      try {
        // 1차: 동일 timestamp TF
        geometry_msgs::TransformStamped tf =
          tfBufferPtr->lookupTransform(costmap_frame, src_frame, in_msg->header.stamp, ros::Duration(0.05));
        tf2::doTransform(*in_msg, cloud_tf, tf);
      } catch (const std::exception &e1) {
        // 2차: 최신 TF(특히 static TF/시뮬에서 유용)
        try {
          geometry_msgs::TransformStamped tf =
            tfBufferPtr->lookupTransform(costmap_frame, src_frame, ros::Time(0), ros::Duration(0.05));
          tf2::doTransform(*in_msg, cloud_tf, tf);
        } catch (const std::exception &e2) {
          ROS_WARN_THROTTLE(1.0, "TF failed (%s -> %s): %s / %s",
                            src_frame.c_str(), costmap_frame.c_str(), e1.what(), e2.what());
          if (require_tf) return;
          cloud_tf = *in_msg; // fallback (⚠ frame mismatch 가능)
        }
      }
    } else {
      cloud_tf = *in_msg;
  ...
  // cloud_tf는 변환 후 frame_id가 들어있을 수 있지만,
  // publish는 out_hdr로 강제해서 프레임을 항상 통일
  // (전제: use_tf=true일 때 의미적으로 일치)
  ...
  // 5) ground removal
  pcl::PointCloud<pcl::PointXYZI>::Ptr nonground_cloud = removeGroundByHeight(roi_cloud);
  if (nonground_cloud->empty()) return;

  // publish nonground (debug)
  {
    sensor_msgs::PointCloud2 out;
    pcl::toROSMsg(*nonground_cloud, out);
    out.header = out_hdr;
  ...
  std::vector<pcl::PointIndices> clusters =
    euclideanCluster(nonground_cloud, cluster_tolerance, cluster_min_size, cluster_max_size);

  // publish all colored clusters
  publishColoredClustersAll(nonground_cloud, clusters, out_hdr);

  // 7) detections
  std::vector<Detection> dets = buildDetections(nonground_cloud, clusters, out_hdr.stamp);

  // messages to publish
  geometry_msgs::PoseArray centers_msg;
  centers_msg.header = out_hdr;
  ...
  initCostmap(costmap, out_hdr);

  // costmap frame 강제
  costmap.header.frame_id = costmap_frame;
  costmap.header.stamp    = in_msg->header.stamp;

  // 9) detections -> cav -> markers + paint costmap
  for (const auto &d : dets) {
    ...
    box.lifetime = ros::Duration(1.0); // RViz 깜빡임 방지
    ...
    if (d.id >= 0 && d.id < (int)clusters.size()) {
      for (int idx : clusters[d.id].indices) {
        if (idx < 0 || idx >= (int)nonground_cloud->size()) continue;
        const auto &src = nonground_cloud->points[idx];
        pcl::PointXYZRGB p;
        p.x = src.x; p.y = src.y; p.z = src.z;
        p.r = 255; p.g = 0; p.b = 0;
    ...
    paintAABB(costmap,
              d.min_pt.x(), d.min_pt.y(),
              d.max_pt.x(), d.max_pt.y(),
              cav_cost_green, inflation_m);
...
int main(int argc, char **argv)
{
  ...
  ROS_INFO(" - use_tf: %s, require_tf: %s, costmap_frame: %s",
           use_tf ? "true":"false", require_tf ? "true":"false", costmap_frame.c_str());
