#include <ros/ros.h>
#include <morai_msgs/EgoVehicleStatus.h> // 타입 호환 위해 포함(미사용)
...
// ======= map(x, y, z) → ENU 회전 정렬 (GPS 노드에서는 직접 사용 안 하지만 유지) =======
...
{
  ros::init(argc, argv, "coord_convert_gps_node");
  ros::NodeHandle nh("~");
  ...
  ROS_INFO_STREAM("[coord_convert_gps_node] origin = ("
