...
void costmapCallback(const nav_msgs::OccupancyGrid::ConstPtr& msg) {
    // Costmap 데이터를 처리하는 로직을 여기에 추가.
    // 예: costmap 데이터를 전역 변수에 저장하거나, 경로 계획에 활용하는 등
...
void controlLoop(const ros::TimerEvent&) {
    ...
    getGoalwaypoint(ego, ctrl.close_idx, ctrl.target_idx, ctrl.ld); // close waypoint에서 ld 보다 먼 target waypoint잡기
