...
// costmap frame_id 저장 (costmap이 base_link면 planner publish도 base_link로 맞추기 위함)
std::string costmap_frame_id = "base_link";
...
double LETHAL_COST_THRESHOLD = 100;  // Lethal obstacle threshold
...
// (ego.x, ego.y, ego.yaw)는 ENU(map) 기준
// costmap은 base_link 좌표(ego 기준 로컬)로 만들어졌으므로,
// global point를 costmap lookup 하려면 base_link로 변환 필요
static inline void mapToBaseLink(double mx, double my, const VehicleState& ego,
                                 double& bx, double& by)
{
    double dx = mx - ego.x;
    double dy = my - ego.y;
...
void costmapCallback(const nav_msgs::OccupancyGrid::ConstPtr& msg) {
    ...
    // costmap frame 저장
    costmap_frame_id = msg->header.frame_id;
}
...
vector<pair<double, double>> generatePolynomialPath(
    ...
    // 시작 yaw 기준 좌표계로 회전 (start가 원점, start heading이 +x)
    double c0 = cos(start_yaw);
    double s0 = sin(start_yaw);

    // global -> start frame
    double x_f =  c0 * dx + s0 * dy;   // rotate by -start_yaw
    double y_f = -s0 * dx + c0 * dy;

    // end_yaw도 start frame 기준으로 변환
    double yaw_f = normalizeAngle(end_yaw - start_yaw);

    // 5차 다항식 y(x) = a0 + a1 x + a2 x^2 + a3 x^3 + a4 x^4 + a5 x^5
    // boundary:
    // y(0)=0, y'(0)=0, y''(0)=0  (start에서 부드럽게)
    // y(X)=Y, y'(X)=tan(yaw_f), y''(X)=0  (end에서도 부드럽게)
    double X = x_f;
    double Y = y_f;

    // 예외: X가 너무 작거나 뒤쪽이면(비정상) fallback
    if (fabs(X) < 1e-3) {
        // 간단한 선형 보간 fallback
    ...
    // y'(X)= 3a3 X^2 + 4a4 X^3 + 5a5 X^4 = yp
    ...
    for (int i = 0; i <= num_points; i++) {
        ...
        // start frame -> global(frame of start_x,start_y,start_yaw)
        double gx = start_x + c0 * x - s0 * y;
        double gy = start_y + s0 * x + c0 * y;
...
bool worldToCostmapCoord(double world_x, double world_y, int& grid_x, int& grid_y) {
    ...
    // (중요) 음수 좌표가 있으므로 floor 사용 (int 캐스팅은 0쪽으로 trunc되어 오차 발생)
    grid_x = (int)std::floor((world_x - costmap_origin_x) / costmap_resolution);
    grid_y = (int)std::floor((world_y - costmap_origin_y) / costmap_resolution);
...
int getCostmapCost(double world_x, double world_y) {
    ...
    if (!worldToCostmapCoord(world_x, world_y, grid_x, grid_y)) {
        return (int)LETHAL_COST_THRESHOLD;  // Out of bounds = 치명적
    }
    // costmap의 1차원 데이터 배열에서 인덱스 계산
    // (grid_x, grid_y) → 1차원 인덱스(idx)로 변환
    int idx = grid_y * (int)global_costmap->info.width + grid_x;
    if (idx < 0 || idx >= (int)global_costmap->data.size()) {
        return (int)LETHAL_COST_THRESHOLD;   // Out of bounds = 치명적
    }
    
    // OccupancyGrid 표준: -1(unknown), 0~100
    int8_t raw = global_costmap->data[idx];

    // unknown이면 보수적으로 lethal로 취급
    // if (raw < 0) return (int)LETHAL_COST_THRESHOLD;
    if (raw < 0) return 30; // unknown을 0~100 중 중간값으로
...
void evaluatePathCollision(CandidatePath& path) {
    ...
    int lethal_count = 0;
    int valid_point_count = 0;  // ← 추가: costmap 범위 안의 점만 카운트
    
    for (const auto& pt : path.points) {
        int grid_x, grid_y;
        
        // 범위 밖이면 무시 (costmap 밖의 영역은 평가 대상에서 제외)
        if (!worldToCostmapCoord(pt.first, pt.second, grid_x, grid_y)) {
            continue;  // ← 범위 밖은 스킵
        }
        
        valid_point_count++;  // ← 범위 안의 점만 카운트
        
        int cost = getCostmapCost(pt.first, pt.second);
        
        if (cost >= (int)LETHAL_COST_THRESHOLD) {
            lethal_count++;
        }
    ...
    // 평균 계산 (범위 안의 점으로만)
    if (valid_point_count > 0) {  // ← valid_point_count로 변경
...
vector<CandidatePath> generateAndEvaluateCandidates(
    ...
    for (int i = 0; i < (int)NUM_OFFSETS; i++) {
        ...
        // 시작점의 yaw 추정
        // (costmap이 base_link이므로 실제 경로 생성은 ego 기준(0,0,0)에서 수행)
        double start_yaw = 0.0;
        
        // 끝점의 yaw (경로 방향)
        double end_yaw = atan2(dir_y, dir_x);

        // end_yaw를 ego 기준(base_link) 상대각으로 변환
        double end_yaw_rel = normalizeAngle(end_yaw - ego.yaw);

        // 목표점을 ego 기준(base_link) 좌표로 변환
        double goal_bx, goal_by;
        mapToBaseLink(goal_x, goal_y, ego, goal_bx, goal_by);
        
        // 경로 생성
        // ego -> offset goal point 까지 경로 생성
        // (costmap frame(base_link)에서 생성하면 충돌평가/시각화가 costmap과 정확히 겹침)
        vector<pair<double, double>> path = generatePolynomialPath(
            0.0, 0.0, start_yaw,
            goal_bx, goal_by, end_yaw_rel,
            SAMPLE_SPACING
        );
...
nav_msgs::Path convertToNavPath(const CandidatePath& path) {
    ...
    nav_path.header.frame_id = "base_link";  // ← "map" 대신 "base_link"로 변경!
    
    for (const auto& pt : path.points) {
        geometry_msgs::PoseStamped pose;
        pose.header.stamp = ros::Time::now();
        pose.header.frame_id = "base_link";  // ← "map" 대신 "base_link"로 변경!
...
visualization_msgs::MarkerArray visualizeCandidatePaths(
    ...
    for (int i = 0; i < (int)candidates.size(); i++) {
        ...
        marker.header.frame_id = "base_link";  // ← "map" 대신 "base_link"로 변경!
...
// 추가: Path를 MarkerArray로 시각화
visualization_msgs::MarkerArray visualizeBestPath(const CandidatePath& best_path) {
    visualization_msgs::MarkerArray markers;
    ...
    line.scale.x = 0.2;  // ← 초록색보다 굵게 (0.1 → 0.2)
    
    // 파란색, 불투명
    line.color.r = 0.0f;
    line.color.g = 0.0f;
    line.color.b = 1.0f;
    line.color.a = 0.8f;  // ← 더 불투명하게
    
    line.lifetime = ros::Duration(0.1);
    
    for (const auto& pt : best_path.points) {
        geometry_msgs::Point p;
        p.x = pt.first;
        p.y = pt.second;
        p.z = 0.1;  // ← z값 높게 (앞에)
    ...
    // 추가: 경로 포인트들을 작은 구체로 표시
    for (size_t i = 0; i < best_path.points.size(); i++) {
        visualization_msgs::Marker sphere;
        sphere.header.frame_id = "base_link";
        sphere.header.stamp = ros::Time::now();
        sphere.ns = "best_path_points";
        sphere.id = i;
        sphere.type = visualization_msgs::Marker::SPHERE;
        sphere.action = visualization_msgs::Marker::ADD;
        
        sphere.pose.position.x = best_path.points[i].first;
        sphere.pose.position.y = best_path.points[i].second;
...
void controlLoop(const ros::TimerEvent&) {
    ...
    // ← 추가: 각 후보 경로의 상태 출력
    for (int i = 0; i < (int)candidates.size(); i++) {
        ROS_INFO("  Candidate[%d]: valid=%d, cost=%.4f, offset=%.2f, points=%zu",
                 i,
                 candidates[i].valid ? 1 : 0,
                 candidates[i].cost,
                 candidates[i].offset,
                 candidates[i].points.size());
    }
